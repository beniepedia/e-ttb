...
export default function Description({ receipt }) {
    ...
    const handleChange = (e) => {
        setData(e.target.name, e.target.name == 'cost' ? currency(e.target.value.replace(/\D/g, '')) : e.target.value)
    }

    const handleClick = () => {
        patch(route('receipts'), {
            preserveScroll: true,
            onSuccess: () => { setDisable(true) },
        })
    }
    return (
        <div>
            <div className='card bg-base-100 rounded-md shadow-md mt-10 '>
                <div className="card-body p-4">
                    ...
                    <p className='text-sm text-slate-400 italic mb-3'>Keterangan tidak bisa diubah jika barang sudah diambil.</p>
                    ...
                    {/* {data.isTaken} */}
                    {
                        !receipt.isTaken && (
                            <div className='mt-3 flex justify-end'>
                                {
                                    disable
                                        ? (<Button className='w-30 btn-sm' handleClick={handleDisable}>EDIT</Button>)
                                        :
                                        (
                                            <div className='flex gap-2'>
                                                <Button className='w-30 btn-sm btn-error' handleClick={handleDisable}>batal</Button>
                                                <Button
